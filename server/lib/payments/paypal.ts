...
function paypalApi(base: string) {
  return axios.create({
    baseURL: base,
...
export const paypalProvider: PaymentProvider = {
  ...
  async createPayment(input: CreatePaymentInput): Promise<PaymentSession> {
    ...
    if (input.isRecurring) {
      ...
      return {
        redirectUrl: subResp.data.links.find((l: any) => l.rel === 'approve')?.href,
    ...
    return {
      redirectUrl: orderResp.data.links.find((l: any) => l.rel === 'approve')?.href,
  ...
  async verifyWebhook(req: Request): Promise<WebhookEvent | null> {
    ...
    const webhookEvent = req.headers['paypal-transmission-sig'] as string
    ...
    try {
      ...
      if (resp.data.verification_status !== 'SUCCESS') return null
