...
describe('email', () => {
    ...
    describe('sendMagicLinkEmail', () => {
        it('sends magic link email', async () => {
            await expect(sendMagicLinkEmail('user@example.com', 'test-token-123')).resolves.not.toThrow();
        });
    });
    describe('sendWelcomeEmail', () => {
        it('sends welcome email', async () => {
            await expect(sendWelcomeEmail('newuser@example.com', 'John')).resolves.not.toThrow();
        });
    });
    describe('sendNewsletterDigestEmail', () => {
        it('sends newsletter digest email', async () => {
            await expect(sendNewsletterDigestEmail('subscriber@example.com', 'Monthly Impact Update', 'Here are the latest updates from our work across Africa.')).resolves.not.toThrow();
        });
    });
});
